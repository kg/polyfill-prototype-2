...
interface IByteReader {
  peek (offset : number) : number | boolean;
  read () : number | boolean;
  skip (distance : number);
  getPosition () : number;
  eof : boolean;
}
...
export function fromCharCode (codePt : number) : string {
...
export function charCodeAt (str : string, idx : number) : number | boolean {  
...
export function makeByteWriter (outputBytes? : Uint8Array, outputIndex? : number) : IByteWriter {
...
export function makeByteReader (bytes : Uint8Array, index? : number, count? : number) : IByteReader {
  ...
  var peek = function peek (offset : number) : number | boolean {
    offset |= 0;
...
export function makeCharacterReader (str : string) : ICharacterReader {
